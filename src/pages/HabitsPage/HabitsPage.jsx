...
function HabitsPage() {
  ...
  const handleAiAdvice = (habit) => {
    // Заглушка – здесь будет запрос к AI-сервису
    alert(` AI-совет для привычки "${habit.name}": Попробуйте делать её в одно и то же время каждый день!`);
    setMenu(null);
  };
  ...
  const renderHabitCard = (habit, isCompleted = false) => (
    <Substrate
      key={habit.id}
      variant="secondary"
      className={`${styles.habitCard} ${isCompleted ? styles.completedCard : ''}`}
    >
      ...
      {menu?.habitId === habit.id && (
        <ContextMenu
          items={[
            { label: 'Получить AI-совет', onClick: () => handleAiAdvice(habit) },
